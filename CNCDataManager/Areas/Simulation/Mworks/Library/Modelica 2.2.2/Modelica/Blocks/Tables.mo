  ...
  protected 
    Real tableID;
	  ...
	  function tableInit
	    ...
	    output Real  tableID;
	   external "C" tableID=  ModelicaTables_CombiTable1D_init_(tableName, fileName, 
	  ...
	  function tableIpo
	    ...
	    output Real value;
	  external "C" value = 
	                     ModelicaTables_CombiTable1D_interpolate(tableID, icol, u); // dymTableIpo1
	    annotation(Library="ModelicaExternalC"); 
	  end tableIpo;
    annotation (
  ...
  equation 
    ...
    for i in 1:n loop
      y[i] = if not tableOnFile and size(table,1)==1 then 
               table[1, columns[i]] else tableIpo(integer(tableID), columns[i], u[i]);          
    end for;
    when initial() then
  ...
  protected 
    Real tableID;
	  ...
	  function tableInit
	    ...
	    input Integer smoothNess;  
	    output Real tableID;
	   external "C" tableID =  ModelicaTables_CombiTable1D_init_(
...
"),   Icon(
  ...
  equation 
    ...
    for i in 1:nout loop
      y[i] = if not tableOnFile and size(table,1)==1 then 
               table[1, columns[i]] else tableIpo(integer(tableID), columns[i], u);
    end for;
    when initial() then
    	tableID = tableInit(if tableOnFile then tableName else "NoName", if tableOnFile then fileName else "NoName", table, smoothness);
    end when;
...
"),   Icon(
  ...
  function tableInit
    ...
    input Integer  smoothness;
    output Real tableID;
  external "C" tableID=  ModelicaTables_CombiTable2D_init_(
                 tableName, fileName, table, size(table, 1), size(table, 2),
                 smoothness);
  annotation(Library="ModelicaExternalC");
  end tableInit;
  ...
  function tableIpo
    ...
    output Real value;
  external "C" value = 
                     ModelicaTables_CombiTable2D_interpolate(tableID, u1, u2); // dymTableIpo2
  annotation(Library="ModelicaExternalC");
  end tableIpo;
  equation 
    if tableOnFile then
    ...
    y = tableIpo(integer(tableID), u1, u2);
    when initial() then
      tableID=tableInit(if tableOnFile then tableName else "NoName", if tableOnFile then fileName else "NoName", table, smoothness); 
    end when;
  end CombiTable2D;
