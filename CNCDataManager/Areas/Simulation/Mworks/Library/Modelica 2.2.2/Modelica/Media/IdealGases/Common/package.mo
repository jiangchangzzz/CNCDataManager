...
and adapted to the Modelica.Media package.
  ...
  constant IdealGases.Common.DataRecord data(name="",MM=0,Hf=0,H0=0,Tlimit=0,alow=fill(0,7),blow=fill(0,2),
  ahigh=fill(0,7),bhigh=fill(0,2),R=0) 
...
");
  ...
  function h_T "Compute specific enthalpy from temperature and gas data; reference is decided by the 
    ...
    //                       zeroDerivative=refChoice) = h_T_MW_der);
  algorithm 
  ...
  function h_T_MW_der
    ...
    input SI.SpecificEnthalpy h_off=h_offset 
      "User defined offset for reference enthalpy, if referenceChoice = UserDefined";
    input Real T_der;
    input Real h_off_der := 0;
    output Real h_der;
  
  algorithm 
    h_der := (if T < data.Tlimit then data.R*(T_der*(data.blow[1]+data.alow[2]*ln(
      T)+T*(data.alow[3]+T*(0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(
      0.25*data.alow[6]+0.2*data.alow[7]*T)))))+T*(data.alow[2]*T_der/T+T_der*(
      data.alow[3]+T*(0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(0.25*
      data.alow[6]+0.2*data.alow[7]*T))))+T*(T_der*(0.5*data.alow[4]+T*(
      1/3*data.alow[5]+T*(0.25*data.alow[6]+0.2*data.alow[7]*T)))+T*
      (T_der*(1/3*data.alow[5]+T*(0.25*data.alow[6]+0.2*data.alow[7]
      *T))+T*(T_der*(0.25*data.alow[6]+0.2*data.alow[7]*T)+0.2*T*data.alow[7]*
      T_der)))))/T-data.R*(T*(data.blow[1]+data.alow[2]*ln(T)+T*(data.alow[3]+T*(
      0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(0.25*data.alow[6]+0.2*
      data.alow[7]*T)))))-data.alow[1])*T_der/T^2 else data.R*(T_der*(data.bhigh[1]
      +data.ahigh[2]*ln(T)+T*(data.ahigh[3]+T*(0.5*data.ahigh[4]+T*(
      1/3*data.ahigh[5]+T*(0.25*data.ahigh[6]+0.2*data.ahigh[7]*T)))))
      +T*(data.ahigh[2]*T_der/T+T_der*(data.ahigh[3]+T*(0.5*data.ahigh[4]+T*(
      1/3*data.ahigh[5]+T*(0.25*data.ahigh[6]+0.2*data.ahigh[7]*T))))
      +T*(T_der*(0.5*data.ahigh[4]+T*(1/3*data.ahigh[5]+T*(0.25*
      data.ahigh[6]+0.2*data.ahigh[7]*T)))+T*(T_der*(1/3*data.ahigh[5]
      +T*(0.25*data.ahigh[6]+0.2*data.ahigh[7]*T))+T*(T_der*(0.25*data.ahigh[6]+
      0.2*data.ahigh[7]*T)+0.2*T*data.ahigh[7]*T_der)))))/T-data.R*(T*(data.bhigh[1]
      +data.ahigh[2]*ln(T)+T*(data.ahigh[3]+T*(0.5*data.ahigh[4]+T*(
      1/3*data.ahigh[5]+T*(0.25*data.ahigh[6]+0.2*data.ahigh[7]*T)))))
      -data.ahigh[1])*T_der/T^2)+(if refChoice == Choices.ReferenceEnthalpy.UserDefined
       then h_off_der else 0);
  end h_T_MW_der;
  ...
  function h_Tlow "Compute specific enthalpy, low T region; reference is decided by the 
    ...
    // annotation (Inline=false,InlineNoEvent=false,
    //            derivative(zeroDerivative=data,
    //                       zeroDerivative=exclEnthForm,
    //                       zeroDerivative=refChoice) = h_Tlow_MW_der);
  algorithm 
  ...
  function h_Tlow_MW_der "Compute specific enthalpy derivative, low T region; reference is decided by the 
    refChoice input, or by the referenceChoice package constant by default"
    import Modelica.Media.Interfaces.PartialMedium.Choices;
    extends Modelica.Icons.Function;
    input IdealGases.Common.DataRecord data "Ideal gas data";
  ...
  algorithm 
    h_der := data.R*(T_der*(data.blow[1]+data.alow[2]*ln(
      T)+T*(data.alow[3]+T*(0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(
      0.25*data.alow[6]+0.2*data.alow[7]*T)))))+T*(data.alow[2]*T_der/T+T_der*(
      data.alow[3]+T*(0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(0.25*
      data.alow[6]+0.2*data.alow[7]*T))))+T*(T_der*(0.5*data.alow[4]+T*(
      ...
      *T))+T*(T_der*(0.25*data.alow[6]+0.2*data.alow[7]*T)+0.2*T*data.alow[7]*
      T_der)))))/T-data.R*(T*(data.blow[1]+data.alow[2]*ln(T)+T*(data.alow[3]+T*(
      0.5*data.alow[4]+T*(1/3*data.alow[5]+T*(0.25*data.alow[6]+0.2*
      data.alow[7]*T)))))-data.alow[1])*T_der/T^2
      +(if refChoice == Choices.ReferenceEnthalpy.UserDefined
       then h_off_der else 0);
  end h_Tlow_MW_der;
...
required from medium model \""   + mediumName + "\".");
  ...
  function h_TX_MW_der
    import Modelica.Media.Interfaces.PartialMedium.Choices;
     ...
     input Real T_der;
     input Real X_der[:] ;
     input Real h_off_der;
     output Real h_der;
  algorithm 
     h_der := (if fixedX then zeros(size(reference_X,1)) else X_der)*
     {SingleGasNasa.h_T(data[i], T, exclEnthForm, refChoice, h_off) for i in 1:nX}
     + (if fixedX then reference_X else X)*
     {SingleGasNasa.h_T_MW_der(data[i], T, exclEnthForm, refChoice, h_off, T_der, h_off_der) for i in 1:nX};
  end h_TX_MW_der;
